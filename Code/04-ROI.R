...
# Read computed data (else: code to replicate)
outfile <- paste0(git_path,"04-ROI/nbr_groups_mono-sim_gt_net01_roi01.rds")
# Subsetting
...
# Save tables and matrices results
...
#show(gg1)
# Save it
#plotfile <- "nbr_groups_mono-sim_gt_net01_roi01_chord.svg"
#ggsave(outfile, gg1, "svg", outdir, 1, 5.5, 4.5)

# Sum edges by hemisphere
hem_tab <- table(atlas$hemi[res_df$idx1],atlas$hemi[res_df$idx2])
hem_vec <- c(c(hem_tab)[c(3,6)],sum(c(hem_tab)[c(4,5)]))
# Plot it
#plotfile <- "nbr_groups_mono-sim_gt_net01_roi01_hemfreq.svg"
#svg(plotfile, 4, 4)
barplot(height = hem_vec,
        names = c("L-L","R-R","L-R"),
        horiz = T,
        las = 1,
        cex.names = 2,
        ...
        cex.main = 2)
#dev.off()
prop.test(x = hem_vec[3], n = sum(hem_vec), correct = F)
